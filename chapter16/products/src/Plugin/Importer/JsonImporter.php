...
class JsonImporter extends ImporterBase {
   ...
   * {@inheritdoc}
   */
  public function __construct(array $configuration, $plugin_id, $plugin_definition, EntityTypeManager $entityTypeManager, Client $httpClient, MessengerInterface $messenger, FileRepositoryInterface $fileRepository) {
   ...
   *   The product entity.
   */
  protected function handleProductImage(array $data, ProductInterface $product) {
    $name = $data->image;
    ...
    $file = $this->fileRepository->writeData($image, 'public://product_images/' . $name, FileSystemInterface::EXISTS_REPLACE);
    if (!$file) {
